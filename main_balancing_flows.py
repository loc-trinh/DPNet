...
def main():
    ...
    parser.add_argument('--record_dir', default='record/stackn/', type=str)
    ...
    data_loader = dataloader.StackNImageLoader(args.batch_size, args.num_workers, data_dir, frame_count,
...
class DPNet():
    ...
    def build_model(self):        
        self.model = create_dpnet_one(in_channel=18)
        self.model = nn.DataParallel(self.model)

        balancing_weight = torch.tensor([2, 0.5]).to(self.device)
        self.criterion = nn.CrossEntropyLoss(balancing_weight)
        self.warm_optimizer = torch.optim.Adam([{'params': self.model.module.add_on_layers.parameters(), 'lr': 2e-4},
                                                {'params': self.model.module.prototype_vectors, 'lr': 2e-4}])
        self.full_optimizer = torch.optim.Adam([{'params': self.model.module.features.parameters(), 'lr': 2e-4},
                                                {'params': self.model.module.add_on_layers.parameters(), 'lr': 2e-4},
                                                {'params': self.model.module.prototype_vectors, 'lr': 2e-4}])
