...
class DPNet(nn.Module):
    def __init__(self, rgb_features, flow_features, img_size, prototype_shape,
        ...
        self.add_on_layers = nn.Sequential(
            nn.Conv2d(in_channels=1440, out_channels=self.prototype_shape[1], kernel_size=1),
    ...
    def conv_features(self, x_rgb, x_flow):
        '''
        the feature input to prototype layer
        '''
        x_rgb = self.rgb_features(x_rgb)
        x_flow = self.flow_features(x_flow)
    ...
    def prototype_distances(self, x_rgb, x_flow):
        conv_features = self.conv_features(x_rgb, x_flow)
    ...
    def forward(self, x_rgb, x_flow):
        distances = self.prototype_distances(x_rgb, x_flow)
    ...
    def foward_prototype_activations(self, x_rgb, x_flow):
        distances = self.prototype_distances(x_rgb, x_flow)
        min_distances = -F.max_pool2d(-distances, kernel_size=(distances.size()[2], distances.size()[3]))
        min_distances = min_distances.view(-1, self.num_prototypes)
        prototype_activations = self.distance_2_similarity(min_distances)
    ...
    def foward_logits(self, x_rgb, x_flow):
        distances = self.prototype_distances(x_rgb, x_flow)
        min_distances = -F.max_pool2d(-distances, kernel_size=(distances.size()[2], distances.size()[3]))
        min_distances = min_distances.view(-1, self.num_prototypes)
        prototype_activations = self.distance_2_similarity(min_distances)
    ...
    def push_forward(self, x_rgb, x_flow):
        '''
        This method is needed for the pushing operation
        '''
        conv_output = self.conv_features(x_rgb, x_flow)
...
def create_dpnet_two(in_channel=10, img_size=256, prototype_shape=(50, 256, 1, 1), num_classes=2):
    rgb_features = create_hrnet(False)
    flow_features = create_hrnet(True)
    
    return DPNet(rgb_features=rgb_features,
