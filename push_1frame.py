...
def push_prototypes(device,
    ...
    for push_iter, (video_names, search_batch_input, search_y) in enumerate(progress):
...
def update_prototypes_on_batch(device, 
    ...
    for j in range(n_prototypes):
        ...
        if batch_min_proto_dist_j < global_min_proto_dist[j]:
            ...
            # get the whole image           
            original_img_j = search_batch_input[img_index_in_batch].clone()
            ...
            if proto_bound_boxes.shape[1] == 6 and search_y is not None:
                proto_bound_boxes[j, 5] = search_y[img_index_in_batch].item()
